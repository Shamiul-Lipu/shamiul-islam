...
const Projects = () => {
  return (
    <section className="bg-primary py-10">
      <Container>
        <div>
          <h3 className="font-bold text-textPrimary text-3xl text-center pb-10">
            Projects
          </h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 py-5 px-2">
          {projetsList.map((project, index) => (
            <div
              key={index}
              className="card w-full bg-[#121c33] hover:bg-[#1a243d]   text-textSecondary shadow-xl hover:-translate-y-4 transition-all hover:delay-150 ease-in-out hover:shadow-2xl hover:shadow-cyan-900"
            >
              ...
              <div className="flex flex-wrap gap-2 justify-center px-10 pt-5 text-white  font-semibold">
                ...
                <Link
                  target="_blank"
                  to={`${project.liveLink}`}
                  className="flex gap-2 px-4 py-2 bg-gradient-to-b from-[#1a243d] to-sky-900 rounded-xl border-b-2 border-b-primary hover:underline hover:border-b-2 hover:text-sky-200 hover:border-blue-400 transition-all"
                >
